...
import {
  ...
  Image as ImageIcon, Camera, X, Calendar, Eye, EyeOff, AlertTriangle,
} from 'lucide-react'
...
export default function ConfiguracoesPage() {
  ...
  async function handleSelecionarFuncao(novaFuncao: string) {
    ...
    // Se não havia função anterior ou não há tenantId, troca direto
    if (!funcaoPrincipal || !tenantId) {
      setFuncaoPrincipal(novaFuncao)
      return
    }

    // Consulta leads ativos antes de exibir banner
    setBannerFunil({ novaFuncao, leadsAtivos: 0, carregando: true })

    const res = await fetch(`/api/crm/trocar-funil?tenant_id=${tenantId}`)
  ...
  async function confirmarTrocaFunil() {
    if (!bannerFunil || !tenantId || !userId || !tenant) return
    ...
    const supabase = createClient()
    const diasStr = horario.dias.map((d: number) => DIAS_NUM_TO_STR[d]).filter(Boolean)
    ...
    await supabase.from('tenants').update({
      horario_funcionamento: { ...horario, funcoes: [novaFuncao] },
    }).eq('id', tenant.id)
  ...
  async function handleSalvar() {
    ...
    const horarioComFuncoes: HorarioFuncionamento = {
      ...horario,
              ...
              <div className="flex-1">
                ...
                {bannerFunil.carregando ? (
                  <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
                    Verificando leads ativos...
                  </p>
                ) : (
                  <div className="mt-1 space-y-1">
                    <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                      Você está trocando de <strong>{LABELS_FUNIL[funcaoPrincipal] ?? funcaoPrincipal}</strong> para{' '}
                      <strong>{LABELS_FUNIL[bannerFunil.novaFuncao] ?? bannerFunil.novaFuncao}</strong>.
                    </p>
                    {bannerFunil.leadsAtivos > 0 ? (
                      <p className="text-xs text-yellow-400 font-medium">
                        ⚠ {bannerFunil.leadsAtivos} lead{bannerFunil.leadsAtivos !== 1 ? 's' : ''} ativo{bannerFunil.leadsAtivos !== 1 ? 's' : ''} será{bannerFunil.leadsAtivos !== 1 ? 'ão' : ''} movido{bannerFunil.leadsAtivos !== 1 ? 's' : ''} para a primeira etapa do novo funil. Os leads encerrados manterão seu histórico.
                      </p>
                    ) : (
            ...
            <div className="rounded-xl p-6" style={cardStyle}>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {FUNCOES.map(f => (
                    <button key={f.id} type="button" onClick={() => handleSelecionarFuncao(f.id)}
              ...
              <div className="rounded-xl p-6" style={cardStyle}>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
                      Cole a chave completa incluindo os cabeçalhos BEGIN/END.
                    </p>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold uppercase tracking-widest mb-1.5" style={{ color: 'var(--text-muted)' }}>
            ...
            <div className="rounded-xl p-6" style={cardStyle}>
              <div className="flex items-center justify-between mb-1">
                ...
                <button onClick={() => fileInputRef.current?.click()} disabled={uploadando}
                  className="flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-50"
                    ...
                    return (
                      <div key={arquivo.id} className="flex items-center justify-between rounded-lg px-4 py-3"
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          {isImg
                            ? <div className="w-6 h-6 flex items-center justify-center flex-shrink-0"><ImageIcon size={16} style={{ color: '#3B82F6' }} /></div>
                        ...
                        <button onClick={() => handleExcluir(arquivo.id, arquivo.nome_arquivo)}
                          ...
                          className="hover:text-red-400 disabled:opacity-40 ml-3 flex-shrink-0"
                          style={{ color: 'var(--text-muted)' }}>
                          <Trash2 size={15} />
                        </button>
                      </div>
