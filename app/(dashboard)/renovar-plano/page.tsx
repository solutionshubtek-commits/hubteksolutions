  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const { data: tenantData } = await supabase
        .from('tenants')
        .select('nome, plano, expira_em')
        .eq('id', userData.tenant_id)
        .single()
      if (tenantData) {
        setTenant(tenantData)
        setPlanoSelecionado(tenantData.plano ?? 'essencial')
      }
      setCarregando(false)
    }
    fetchData()
  ...
  return (
    <div className="flex justify-center px-4 py-8">
      <div className="w-full max-w-2xl space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Renovar Plano</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
            Selecione o plano e período desejados. Enviaremos o link de pagamento via WhatsApp.
          </p>
        </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-semibold" style={{ color: expirado ? '#EF4444' : expirando ? '#F59E0B' : 'var(--text-primary)' }}>
                {tenant?.expira_em
                  ? new Date(tenant.expira_em).toLocaleDateString('pt-BR')
                  : '—'}
                {expirado && ' · Expirado'}
                {expirando && ` · ${dias} dias restantes`}
              </p>
            </div>
          </div>
        </div>
              ...
              return (
                <button key={p.value} type="button" onClick={() => setPlanoSelecionado(p.value)}
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {isAtual && (
                      <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: '#10B98120', color: '#10B981' }}>
                        atual
                      </span>
                    )}
                  </div>
                  <p className="text-xs" style={{ color: 'var(--text-muted)' }}>até {p.limite} conversas</p>
                  <p className="text-sm font-bold mt-1" style={{ color: 'var(--text-primary)' }}>{fmtBRL(p.valor)}<span className="text-xs font-normal" style={{ color: 'var(--text-muted)' }}>/mês</span></p>
                </button>
              )
            })}
          </div>
                ...
                <button key={p.value} type="button" onClick={() => setPeriodoSelecionado(p.value)}
                  ...
                  {p.desconto > 0 && (
                    <span className="inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded-full mt-1" style={{ background: '#10B98120', color: '#10B981' }}>
                      -{Math.round(p.desconto * 100)}% desconto
                    </span>
                  )}
        ...
        {/* Resumo */}
        <div className="rounded-xl p-5" style={{ background: '#10B98108', border: '1px solid #10B98130' }}>
        ...
        <div className="flex items-start gap-2 p-3 rounded-lg" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
          ...
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
            Ao clicar, você será direcionado ao WhatsApp com sua solicitação pré-preenchida. Nossa equipe enviará o link de pagamento em instantes.
          </p>
        </div>
