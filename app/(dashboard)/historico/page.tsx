        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Buscar por nome ou telefone..." value={busca}
              ...
              className="w-full rounded-lg pl-9 pr-4 py-2.5 text-sm focus:outline-none" style={inputStyle} />
          </div>
          <div className="flex gap-2">
            <input type="date" value={dataInicio} onChange={(e) => setDataInicio(e.target.value)}
              className="flex-1 sm:flex-none rounded-lg px-3 py-2.5 text-sm focus:outline-none" style={inputStyle} />
                ...
                {conversasFiltradas.map(c => (
                  <div key={c.id} className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-1 text-xs" style={{ color: 'var(--text-secondary)' }}>
                      ...
                      <div><span style={{ color: 'var(--text-muted)' }}>Fim: </span>{formatarData(c.ultima_mensagem_em)}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input type="text" placeholder="Buscar no log..." value={buscarLog}
                onChange={(e) => setBuscarLog(e.target.value)}
                className="w-full rounded-lg pl-8 pr-4 py-2 text-sm focus:outline-none" style={inputStyle} />
            </div>
          </div>
